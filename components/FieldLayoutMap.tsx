...
export default function FieldLayoutMap({ location, trials, harvestId, onClose, onFinalize }: { location: { id: string; name: string; city: string; lat: number; lng: number }; trials: LayoutTrial[]; harvestId: string; onClose: () => void; onFinalize?: (payload: LayoutFinalization) => void }) {
  ...
  const [isEditing, setIsEditing] = useState(() => !savedSnapshot);
  ...
  const [detectionThreshold, setDetectionThreshold] = useState(savedMetadata?.detectionThreshold ?? 250);
  ...
  const colors = Object.fromEntries(trialIds.map((id, index) => [id, id === 'PULV' ? '#212121' : allocations.find((allocation) => allocation.id === id)?.color ?? palette[index % palette.length]]));
  const maxRow = Math.max(1, ...plots.map((plot) => plot.row));
  const maxCol = Math.max(1, ...plots.map((plot) => plot.col));
  const trialName = (id: string) => trialLabels[id]?.trim() || trials.find((trial) => trial.id === id)?.name || (id === 'PULV' ? 'Pista Pulverização' : `Ensaio ${id}`);
  ...
  const firstPlotByTrial = useMemo(() => {
    ...
    plots.filter((plot) => !isUtilityPlot(plot)).forEach((plot) => { const current = first[plot.trialId]; if (!current || plot.row < current.row || (plot.row === current.row && plot.col < current.col)) first[plot.trialId] = plot; });
    return first;
  }, [plots]);
  ...
  function removeAllocation(id: string) {
    setImportedPlots(plots.map((plot) => plot.allocationId === id ? { ...plot, trialId: plot.sourceTrialId ?? plot.trialId, allocationId: undefined } : plot));
      ...
      <div className="layout-workspace">
        ...
        <div className="layout-map">
          <MapContainer key={`${location.id}-${center[0]}-${center[1]}`} center={center} zoom={19} minZoom={3} maxZoom={24} zoomSnap={.25} zoomDelta={.5} wheelPxPerZoomLevel={40} scrollWheelZoom doubleClickZoom boxZoom preferCanvas className="leaflet-map layout-leaflet">
            ...
            {polygons.filter(({ plot }) => trialVisibility[plot.trialId] !== false).map(({ plot, positions, plotCenter }) => { const issue = plotIssues[plot.plotId]; const selected = selectedPlotId === plot.plotId; const showMapLabel = !isUtilityPlot(plot); return <Fragment key={`${plot.plotId}-${plot.row}-${plot.col}`}><Polygon positions={positions} eventHandlers={{ click: () => selectPlot(plot.plotId) }} pathOptions={{ color: selected ? '#00d4ff' : issue ? issueColor(issue) : '#ffffff', weight: selected ? 3.5 : issue ? 2.4 : .7, fillColor: colors[plot.trialId] ?? '#4d885e', fillOpacity: selected ? .9 : .7 }}><Tooltip sticky><b>Parcela {plot.plotId}</b><br />Ensaio: {trialName(plot.trialId)}<br />Linha {plot.row} · Coluna {plot.col}<br />{plot.genotype}{issue && <><br /><strong style={{ color: issueColor(issue) }}>{issue.kind}</strong><br />{issue.note}{issue.replacementGenotype && <><br />Novo genótipo: {issue.replacementGenotype}</>}</>}</Tooltip></Polygon>{showMapLabel && showPlotIds && mapZoom >= 18 && <Marker position={plotCenter} interactive={false} icon={divIcon({ className:'layout-plot-id-icon', html:`<span>${escapeHtml(plot.plotId)}</span>`, iconSize:[54,16], iconAnchor:[27,8] })} />}{issue && <Marker position={plotCenter} interactive={false} zIndexOffset={900} icon={divIcon({ className:`layout-issue-icon ${issue.kind === 'Observação' ? 'note' : 'alert'}`, html:'<span>!</span>', iconSize:[20,20], iconAnchor:[-8,10] })} />}{showMapLabel && firstPlotByTrial[plot.trialId]?.plotId === plot.plotId && <Marker position={positions[0]} interactive={false} zIndexOffset={1000} icon={divIcon({ className:'layout-trial-start-icon', html:`<span>${escapeHtml(trialName(plot.trialId))}</span>`, iconSize:[140,26], iconAnchor:[0,26] })} />}</Fragment>; })}
          </MapContainer>
